...
def render_map_panel():
    """Left column: main MSR map and house detail map."""
    ...
    # When a new MSR is clicked: load data and trigger full app rerun
    if map_data.get("last_object_clicked_tooltip"):
        new_id = map_data["last_object_clicked_tooltip"]
        if new_id != st.session_state.last_msr_id:
    ...
    # House detail map for the selected MSR
    if st.session_state.selected_id and st.session_state.cached_df is not None:
...
def render_analysis_panel():
    """Right column: MSR info, charging strategy, date controls, chart and KPIs.

    Widget interactions here only re-run this fragment, not the map.
    """
    ...
    # --- MSR header with reverse-geocoded address ---
    try:
        ...
        st.subheader(f"MSR: {st.session_state.selected_id}")
        st.caption("(Adres kon niet worden opgehaald)")
    ...
    EV_adoption_perc = st.slider(
        "Welk percentage EV-adoptie wil je modelleren?",
    ...
    # Store original peak power on first load of this MSR (at current EV %)
    ...
    if st.button("Verander naar dag met hoogste piekvermogen"):
        st.session_state.date_max_power = df_output.loc[df_output["MSR totaal [kW]"].idxmax(), "DATUM_TIJDSTIP_2024"]
        st.session_state.min_max = "max"

    if st.button("Verander naar dag met laagste (of meest negatieve) piekvermogen"):
        st.session_state.date_min_power = df_output.loc[df_output["MSR totaal [kW]"].idxmin(), "DATUM_TIJDSTIP_2024"]
